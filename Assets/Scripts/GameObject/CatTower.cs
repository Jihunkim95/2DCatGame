...
public class CatTower : MonoBehaviour
{
    ...
    [Header("스프라이트 설정")]
    public float pixelsPerUnit = 32f; // 픽셀 퍼 유닛 설정 (Inspector에서 조정 가능)
    public Vector2 spritePivot = new Vector2(0.5f, 0.5f); // 스프라이트 피벗
    ...
    public void CreateTowerSprite()
    {
        ...
        {
            // 기존 텍스처에서 스프라이트 생성 시 올바른 PixelsPerUnit 사용
            ...
            Sprite towerSprite = Sprite.Create(
                ...
                spritePivot,
                actualPixelsPerUnit // 동적으로 계산된 값 사용
            );

            spriteRenderer.sprite = towerSprite;
            spriteRenderer.color = GetLevelColor();

            Debug.Log($"타워 이미지 로드 성공 - 레벨 {level} (크기: {towerTexture.width}x{towerTexture.height}, PPU: {actualPixelsPerUnit})");
            DebugLogger.LogToFile($"타워 이미지 로드 성공 - 레벨 {level} (크기: {towerTexture.width}x{towerTexture.height}, PPU: {actualPixelsPerUnit})");
        }
        else
        {
    ...
    {
        // 텍스처 크기에 따라 적절한 PixelsPerUnit 계산
        // 또는 Inspector에서 설정한 값 사용

        if (pixelsPerUnit > 0)
        {
            // Inspector에서 설정한 값이 있으면 사용
            return pixelsPerUnit;
        }
        else
        {
            // 텍스처 크기에 따라 자동 계산
            // 일반적으로 64x64 텍스처는 32 PPU, 128x128은 64 PPU 등
            float averageSize = (texture.width + texture.height) / 2f;
            return Mathf.Max(averageSize / 2f, 16f); // 최소 16 PPU
        }
    }
    ...
    void CreateProceduralTowerSprite()
    {
        ...
        // 프로그래밍 방식 스프라이트에도 올바른 PixelsPerUnit 적용
        float proceduralPixelsPerUnit = pixelsPerUnit > 0 ? pixelsPerUnit : 32f; // 기본값 32
        ...
        Sprite sprite = Sprite.Create(
            ...
            spritePivot,
            proceduralPixelsPerUnit // Inspector에서 설정한 값 사용
        );

        spriteRenderer.sprite = sprite;
        spriteRenderer.color = Color.white; // 이미 색상이 적용되었으므로 흰색 사용

        Debug.Log($"프로그래밍 방식 타워 스프라이트 생성 완료 - 레벨 {level}, PPU: {proceduralPixelsPerUnit}");
    }
